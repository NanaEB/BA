...
<!--_______________________________________ROOT_TEMPLATE_______________________________________-->
	...
	<xsl:template match="fsm:FSM">
		<xsl:text disable-output-escaping="yes"> <!--um boolische ausdrucke zuausgeben-->
#define FSM_DEBUG      
#include &lt;stdio.h&gt;
#include &lt;string.h&gt;
#include &lt;omnetpp.h&gt;
...
<xsl:template name="printExitCaseBody">
<xsl:for-each select="outTrans">  <xsl:if test="string-length(@Guard) != 0">				
								if(<xsl:value-of select="@Guard" disable-output-escaping="yes"/>){</xsl:if>
								  ...
								  //effect
								  <xsl:value-of select="@Effect" disable-output-escaping="yes"/>;</xsl:if>
...
<xsl:template name="printEnterCase">
			...
			<xsl:choose>
   			<xsl:when test="entry/@entryLabel != ''">
   								//entry code
    		  					<xsl:value-of select="entry/@entryLabel" disable-output-escaping="yes"/>;</xsl:when>
			</xsl:choose>   
								break; 
</xsl:template>
...
<xsl:template name="printExitCode"> 
 <xsl:if test="string-length(exit/@exitLabel) != 0">
 								//exit code
								<xsl:value-of select="exit/@exitLabel" disable-output-escaping="yes"/>;
</xsl:if>
</xsl:template>
